...
OSSim   *osSim=0;    //osSim ÊÇÒ»¸öÈ«¾Ö±äÁ¿
...
/*Ö÷Òª×÷ÓÃÊÇ¶Ô²ÎÊý½øÐÐ´¦Àí£¬Èç¹ûSESC_ENERGY ¶¨ÒåÁË£¬Ôò³õÊ¼»¯ÄÜÁ¿×ÓÏµÍ³*/
...
{ 
  ...
  tmp[0] = 0;
  for(int32_t i = 0; i < argc; i++) {  //°Ñ²ÎÊý¶¼·Åµ½tmp ÖÐ
    strcat(tmp,argv[i]);
    strcat(tmp," ");
  }
...
#else
  processParams(argc,argv,envp); //¶Ô²ÎÊý½øÐÐ´¦Àí,³õÊ¼»¯Êä³öÎÄ¼þ ,INstruction::initization
#endif
#if (defined TLS)
  ...
  EnergyMgr::init();           //Ôø¾­ÔÚÕâÀï³öÏÖ¹ýÒ»¸ö´óbug£¬£¬Ô½²»ÆðÑÛµÄµØ·½Ô½ÈÝÒ×³ö´í
#endif
}
...
{ 
  ...
  char **nargv = (char **)malloc((20+argc)*sizeof(char *)); //ÎªÖ¸Ïò²ÎÊýµÄÖ¸ÕëÉêÇë¿Õ¼ä
  int32_t nargc;
  
  nargv[0] = strdup(argv[0]); //¿½±´argv[0]addressµÄÄÚÈÝ¸ønargv[0] address

  int32_t i=1;   //´ÓµÚÒ»¸ö²ÎÊý¿ªÊ¼´¦Àí  µÚ0¸öÊÇÃüÁîsesc.mem ²»ÐèÒª´¦Àí
  int32_t ni=1;
  ...
  const char *confName=0;  //ÅäÖÃÎÄ¼þÃû
  ...
  for(; i < argc; i++) {     //¶Ô²ÎÊý½øÐÐÊáÀí
    if(argv[i][0] == '-') {
      if( argv[i][1] == 'w' ){
        ...
        trace_flag = true;
      }else{  //¶Ô ÆäËüµÄ²ÎÊý½øÐÐ´¦Àí£¬¶¼·ÅÈë nargvÖÐ
  ...
  Report::openFile(finalReportFile);                      //Êä³öÎÄ¼þ½øÐÐ´¦Àí
...
void OSSim::report(const char *str)
{
  ...
  for(size_t i=0;i<cpus.size();i++) {   //Ã¿¸öcpuµÄÄÜÁ¿ÏûºÄ
    ...
    //å*å*å*×ª»¯³É¹¦ºÄÊ±£¬²»ÊÇÒÔÃ¿¸öcpuÊµ¼ÊÖ´ÐÐµÄclock¼ÆËã£¬¶øÊÇÒÔÔËÐÐÊ±¼ä×î³¤µÄcpuµÄclock¼ÆËã
